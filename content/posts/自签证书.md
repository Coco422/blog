---
title: 自签证书
description: 在开发环境中使用自签名证书为后端提供 HTTPS 时，浏览器访问出现 net::ERR_CERT_AUTHORITY_INVALID 的原因，grok3给出解决方案。文中详细讲解如何使用 mkcert 生成本地信任的证书，让前端页面在无域名、仅使用 IP 的情况下也能安全访问后端接口，并补充手动信任

...
2.  查看已删除文件未被释放的存储空间。
    ...
    输出第7列为文件大小（单位Byte），累加可计算未释放空间总量。![lsof 查找未释放文件的输出](https://imgbed.anluoying.com/2026/03/98f0b96ed2bc0c1565e8ff0b5a1a824c.png)
...
2.  卸载文件系统。
    ...
    **重要**
    ...
    `<重复挂载目录>`可通过[场景四的挂载点检查](#场景四挂载点被覆盖)获取。
...
3.  清理无用文件。
    ...
    > 若Docker文件无法清理，可尝试依照[场景一：磁盘空间耗尽](#场景一磁盘空间耗尽)处理问题。
